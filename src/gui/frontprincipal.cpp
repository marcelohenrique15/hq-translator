    ...
    ui(new Ui::frontprincipal)
{
    ...
    connect(m_cameraThread, &QThread::started, m_cameraWorker, &CameraWorker::process);
    // 2. Quando o worker emitir um frame, chame o nosso método para atualizar a tela
    connect(m_cameraWorker, &CameraWorker::frameReady, this, &frontprincipal::updateCameraView);
    // 3. Quando o worker terminar, agende a thread para fechar de forma segura
    connect(m_cameraWorker, &CameraWorker::finished, m_cameraThread, &QThread::quit);
...
frontprincipal::~frontprincipal()
{
    ...
    if(m_cameraThread && m_cameraThread->isRunning()) {
        m_cameraWorker->stop(); // Avisa ao loop para parar
